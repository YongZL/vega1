...
const approvaStatus1 = [
	{ text: '待验收', value: 'pending' },
	{ text: '验收中', value: 'receiving' },
];
...
const PickingList: FC<{ global: GlobalModelState }> = ({ global }, props) => {
	...
	const [goodsRequestId, setGoodsRequestId] = useState<number>();
	...
	const columns: ProColumns<AcceptanceController.ListRecord>[] = [
		{
			...
			key: 'index',
			align: 'center',
			renderText: (text, redord, index) => index + 1,
			width: 'XXS',
		},
		{
			title: '状态',
			...
			render: (text: any) => (
				<>
					<Badge
						color={approvaStatus[text] && approvaStatus[text].color}
						text={approvaStatus[text] && approvaStatus[text].text}
					/>
				</>
			),
		},
			...
			render: (id, record) => {
				return (
					<div
						...
						{['pending', 'receiving', 'accept_pending', 'accepting'].includes(record.status) &&
							access.check_acceptance_order &&
							(record.acceptType === 2 ? (
								<>
									<Divider
										type='vertical'
	...
	return (
		<div className='main-page'>
			...
			<Card bordered={false}>
				<ProTable
					tableInfoCode='acceptance_order_list'
					loadConfig={{
						request: false,
					}}
					columns={columns}
