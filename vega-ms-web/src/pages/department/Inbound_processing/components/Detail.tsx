...
const handleTitle = {
	...
	approve: '调拨单审核',
	acceptance: accessNameMaplist.check_acceptance_order,
};
...
const DetailModal: React.FC<UpdateProps> = (props) => {
	...
	const scanSubmit = async (valueInput: FormEvent<HTMLInputElement> | string) => {
		...
		const params: ReallocateController.PassParams = {
			operatorBarcode: valueInput,
	...
	const goodsModal: ProColumns<GoodsList>[] = [
		...
		},
		{
			...
			renderText: (text) => {
				return (
					<span>
						<Badge color={reallocationOrderStatusValueEnum[String(text)].color} />
						{reallocationOrderStatusValueEnum[String(text)].label}
					</span>
				);
			},
		},
	...
	const selectRow = (selectData: GoodsList, status: boolean) => {
		...
		if (status) {
			...
			selectedRowKeys.forEach((val, index) => {
				if (val === selectData.operatorBarcode) {
								...
								<ProTable
									...
									toolBarRender={() => [
										handleType === 'acceptance' && (
											<div
												className='scanInput'
												style={{ marginTop: 0, marginBottom: -1, width: 300 }}>
												<ScanInput
													value={scanValue}
													onSubmit={scanSubmit}
