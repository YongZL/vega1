...
const BusinessLicense = ({
	...
	useEffect(() => {
		// gsp配置，ds环境不需要表单默认值
		setIsRequired(WEB_PLATFORM === 'DS' ? undefined : gspEnabled ? true : false);
		setIsLongTimeValid(gspEnabled ? gspEnabled : false);
	}, [gspEnabled]);
	...
	const fealSelectOnChange = (e: RadioChangeEvent) => {
	...
	useEffect(() => {
		if (data) {
			...
			form.setFieldsValue({
				[`${submitPrefix}&isLongTimeValid_licenseEndTime_licenseBeginTime`]: timeVal,
			});
		}
		// if (Object.keys(data).length == 0 && !gspEnabled) {
		//   setIsLongTimeValid(undefined);
		//   form.setFieldsValue({
		//     [`${submitPrefix}&isLongTimeValid_licenseEndTime_licenseBeginTime`]: undefined,
		//   });
		// }
	}, [data]);

	const handleLongTimeValidCheckBoxChange = (e: CheckboxChangeEvent) => {
		if (e) setIsLongTimeValid(e.target.checked);
		form.setFieldsValue({
			[`${submitPrefix}&licenseBeginTime`]: [
				form.getFieldValue(`${submitPrefix}&licenseBeginTime`)[0],
				null,
			],
		});
	};
		...
		nodes.map((item) => {
			...
			const title =
				index > -1 ? (
					<span>
						{beforeStr}
						<span style={{ color: CONFIG_LESS['@c_starus_early_warning'] }}>{searchValue}</span>
						{afterStr}
					</span>
				) : (
					<span>{item.name}</span>
	...
	return (
		<Row gutter={16}>
			<Col {...COL_WIDTH_CONFIG.D}>
				<Row gutter={16}>
					...
					<Col {...COL_WIDTH_CONFIG.F}>
						<FormItem
							label='生效日期'
							name={`${submitPrefix}&licenseBeginTime`}
            ...
            <Col {...COL_WIDTH_CONFIG.F}>
              ...
              >
                <RangePicker style={{ width: '100%' }} format={['YYYY-MM-DD', 'YYYY/MM/DD']} />
              </FormItem>
            </Col>
          )} */}
					...
					<Col {...COL_WIDTH_CONFIG.F}>
						<FormItem
							label='长期有效'
							name={`${submitPrefix}&isLongTimeValid_licenseEndTime_licenseBeginTime`}
							valuePropName='checked'
							initialValue={isLongTimeValid}>
							<Checkbox onChange={handleLongTimeValidCheckBoxChange}></Checkbox>
						</FormItem>
					</Col>
										...
										{isshowKeys12 && (
											<FormItem
												...
												rules={[
													{
														required: gspEnabled
															? checkedKeys18 && checkedKeys18.length <= 0
															: false,
														message: '请选择',
													},
												]}
										...
										{isshowKeys18 && (
											<FormItem
												...
												rules={[
													{
														required: gspEnabled
															? checkedKeys12 && checkedKeys12.length <= 0
															: false,
														message: '请选择',
													},
												]}
                    ...
                    <Radio value={false}>无</Radio>
									...
									{isRequired && (
										<Tree
											...
											onCheck={treeCheck95}
											checkedKeys={
												goodsCategory95 && goodsCategory95.length > 0 ? checkedKeys95 : []
											}
											className='treeBox'
