...
const PermitLicense = ({
	...
	useEffect(() => {
		if (data) {
			...
			form.setFieldsValue({
				[`${submitPrefix}&isLongTimeValid_permitEndTime`]: timeVal,
			});
		}
	}, [data]);
	...
	useEffect(() => {
		...
		if (Object.keys(data).length == 0) {
			setFieldsValue({
				[`${submitPrefix}hasTree`]: gspEnabled ? true : undefined,
			});
			setCheckedKeys18([]);
			setCheckedKeys12([]);
		} else if (
				...
				<Row gutter={16}>
					...
					<Col {...COL_WIDTH_CONFIG.F}>
						<FormItem
							...
							initialValue={
								data.permitBeginTime ? moment(new Date(data.permitBeginTime)) : undefined
							}>
							<DatePicker
								style={{ width: '100%' }}
					...
					<Col {...COL_WIDTH_CONFIG.F}>
						<FormItem
							...
							name={`${submitPrefix}&isLongTimeValid_permitEndTime`}>
					...
					{form.getFieldValue(`${submitPrefix}hasTree`) && (
						<>
							<Col {...COL_WIDTH_CONFIG.C}>
								<FormItem
									label='12版分类'
