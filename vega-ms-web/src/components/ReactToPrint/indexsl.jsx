...
export default class ReactToPrint extends React.Component {
	...
	handlePrintProxy = () => {
		// 待打印目标组件生成前的钩子
		const { onBeforeContentGenerate } = this.props;
		if (onBeforeContentGenerate) {
			...
			sheet.insertRule('body  {@page{size: 35cm 20cm;}}', 0);
		} else {
			this.handlePrint();
		}
	};
	...
	handlePrint = () => {
		const { bodyClass = '', content, copyStyles = true } = this.props;
		...
		printWindow.className = 'pagePrint';
		...
		printWindow.onload = () => {
			...
			// pageStyle.innerHTML = '@page {size: 35cm 20cm;} !important';
			//   cssPagedMedia.size = function (size) {
			//     cssPagedMedia('@page {size: 35cm 20cm;} !important');
			// };
			...
			const defaultPageStyle =
				pageStyle ||
				'@page { margin: 0mm; } @media print { body { -webkit-print-color-adjust: exact; } }';

			const styleEl = domDoc.createElement('style');
			// styleEl.innerHTML='@media print {@page {size: 35cm 20cm;}}'
				...
				for (let index = 0, l = headEls.length; index < l; ++index) {
					...
					if (node.tagName === 'STYLE') {
						...
						if (sheet) {
							...
							if (window.location && window.location.pathname == '/finance/materialReceipt') {
								sheet.insertRule('body  {@page{size: 35cm 20cm;}}', 0);
							} else {
								sheet.insertRule('body {@page{size: auto}} ', 0);
							}
						}
					} else {
