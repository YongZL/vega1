						...
						<tr className={styles.noBorder}>
							<td colSpan='7'>
								<p style={{ fontWeight: 500, fontSize: 24, textAlign: 'center' }}>
									{hospitalName}按{fields.goodsType}进货、领用、结存报表（一）
								</p>
							</td>
						</tr>
						<tr
						...
						{procurementUseList && procurementUseList.length ? (
							<>
								<tr
									...
									<td rowspan='2'>待付款(元)</td>
									<td rowspan='2'>往期待付款(元)</td>
								...
								{(procurementUseList || []).map((item, index) => {
									return (
										<tr
											...
											<td>{convertPriceWithDecimal(item.pendingPay)}</td>
											<td>{convertPriceWithDecimal(item.pendingPayYest)}</td>
